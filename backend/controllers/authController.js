...
export const updateUserRole = async (req, res) => {
  try {
    ...
    if (!userId || !role) {
      return res.status(400).json({
        success: false,
        message: 'Kullanıcı ID ve rol bilgisi gerekli'
      });
    }
...
export const updateUserPassword = async (req, res) => {
  try {
    ...
    // Kullanıcının tüm session'larını temizle (güvenlik için)
    await query(
    ...
    res.json({
      success: true,
      message: 'Kullanıcı şifresi başarıyla güncellendi. Kullanıcının tekrar giriş yapması gerekecek.'
    });
  } catch (error) {
    console.error('Kullanıcı şifresi güncelleme hatası:', error);
    res.status(500).json({
      success: false,
      message: 'Sunucu hatası'
    });
